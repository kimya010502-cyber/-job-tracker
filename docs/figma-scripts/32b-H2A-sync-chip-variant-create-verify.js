...
const SCRIPT_VERSION = '32b-H2A-v1-sync-chip-variant-create-verify';
...
else if (baseline.scriptVersion !== '32-H2A-v1-sync-chip-variant-create') notes.push('기준값 scriptVersion 이 예상과 다르다: ' + baseline.scriptVersion);
...
c.onlyToneAndLeadingProperties = Object.keys(defs).length === 2 && !!toneKey && !!leadingKey;
...
c.syncLeadingIconDotVisible = !!lead && lead.type === 'INSTANCE' && lead.visible === true && !!lmc && lmc.id === IDS.iconDot && near(lead.width, 16) && near(lead.height, 16) &&
  !!lead.componentPropertyReferences && lead.componentPropertyReferences.mainComponent === leadingKey && !!srcLead && kids(nv).indexOf(lead) === kids(src).indexOf(srcLead);
c.syncDotSuccessStrong = isBound(glyph, COLORS.dot);
...
return out({
  ...
  syncVariant: nv ? { id: nv.id, size: size(nv), padding: [nv.paddingTop, nv.paddingRight, nv.paddingBottom, nv.paddingLeft], gap: nv.itemSpacing,
    label: lbl ? lbl.characters + ' ' + r2(lbl.width) : null, leading: lead ? (lmc ? lmc.name : null) + ' ' + size(lead) + ' visible ' + lead.visible : null } : null,
