...
const SCRIPT_VERSION = '19a-v3-phaseD-audit';
...
if (set && set.type === 'COMPONENT_SET') {
  ...
  for (const v of set.children) {
    ...
    /* v2 는 "caption 이라는 이름의 노드가 없으면 optional" 로 봤다. 틀렸다.
     * 노드가 있고 **기본값이 숨김** 이면 optional 이다 — 그게 마스터가 caption 을 선택적으로 만든 방식이다. */
    vrec.captionNodeExists = !!vrec.captionNode;
    vrec.captionVisibleByDefault = vrec.captionNode
      ? (function () {
          const cn = collectDeep(v, x => x.id === vrec.captionNode.id, 6)[0];
          return cn ? cn.visible !== false : null;
        })()
      : null;
    vrec.captionIsOptional = vrec.captionNodeExists && vrec.captionVisibleByDefault === false;
    vrec.captionOptionalBasis = !vrec.captionNodeExists
      ? 'caption 노드 자체가 없다'
      : (vrec.captionVisibleByDefault === false
          ? 'caption 노드가 있고 기본이 숨김이다 — 마스터가 선택적으로 만들어 뒀다'
          : 'caption 노드가 기본 노출이다 — 안 쓰려면 따로 숨겨야 한다');

    /* caption 을 켜고 끌 때 카드 높이가 어떻게 되는지 — 구조에서 계산한다 */
    const capNode = vrec.captionNode
      ? collectDeep(v, x => x.id === vrec.captionNode.id, 6)[0] : null;
    if (capNode) {
      const capParent = capNode.parent;
      const cpGap = capParent && 'itemSpacing' in capParent ? r2(capParent.itemSpacing) : 0;
      const cpMode = capParent && 'layoutMode' in capParent ? capParent.layoutMode : null;
      const capH = r2(capNode.height);
      /* 세로 배치에서만 caption 이 높이를 더한다. 가로면 높이에 영향이 없다. */
      const delta = cpMode === 'VERTICAL' ? r2(capH + cpGap) : 0;
      vrec.captionGeometry = {
        captionId: capNode.id, captionHeight: capH,
        containerId: capParent ? capParent.id : null,
        containerLayoutMode: cpMode, containerGap: cpGap,
        heightContributionWhenVisible: delta,
        masterHeightNow: r2(v.height),
        heightIfCaptionHidden: vrec.captionVisibleByDefault === false
          ? r2(v.height) : r2(v.height - delta),
        heightIfCaptionVisible: vrec.captionVisibleByDefault === false
          ? r2(v.height + delta) : r2(v.height),
        basis: cpMode === 'VERTICAL'
          ? 'caption 은 세로 배치 안에 있어 높이 ' + capH + ' + gap ' + cpGap + ' 만큼 카드 높이를 바꾼다'
          : 'caption 이 세로 배치가 아니라 카드 높이에 더해지지 않는다'
      };
    } else vrec.captionGeometry = null;
...
if (master) {
  for (const c of cards) {
    ...
    /* 신호 2 — 문구 */
    const text = [(m.caption || ''), (m.badgeText || '')].join(' ');
...
/* ---------- caption 처리 방법 비교 ----------
 * 임의로 정하지 않는다. 마스터 구조에서 두 방법의 결과를 각각 계산하고 비교한다. */
let captionHandling = null;
if (master && master.variants.length) {
  const v0 = master.variants[0];
  const cg = v0.captionGeometry;

  const options = [];
  if (cg) {
    options.push({
      option: 'A', what: 'caption 텍스트를 빈 문자열("")로 두고 노드는 보이게 유지',
      resultingCardHeight: cg.heightIfCaptionVisible,
      footerRowHeight: null,
      captionOccupiesSpace: true,
      reproducesNoCaption: false,
      reason: '빈 문자열이어도 TEXT 노드는 line-height ' + cg.captionHeight +
              ' 만큼 자리를 계속 차지한다. 카드가 ' + cg.heightIfCaptionVisible +
              ' 이 되고 배지 위에 빈 줄이 생긴다 — "caption 없음" 을 재현하지 못한다.',
      safeAsInstanceOverride: true,
      overrideNote: 'characters 는 인스턴스에서 덮어쓸 수 있다'
    });
    options.push({
      option: 'B', what: 'caption 노드를 visible = false',
      resultingCardHeight: cg.heightIfCaptionHidden,
      captionOccupiesSpace: false,
      reproducesNoCaption: true,
      reason: 'Auto Layout 이 숨긴 자식을 제외하므로 그 줄이 통째로 사라진다. ' +
              '카드 높이 ' + cg.heightIfCaptionHidden + ' — 현재 "caption 없음 + 배지만" 구조와 같은 모양이다.',
      safeAsInstanceOverride: true,
      overrideNote: 'visible 은 인스턴스에서 덮어쓸 수 있다. ' +
                    '다만 Chip 때 노드 교체로 visible 오버라이드가 초기화된 전례가 있으니 ' +
                    'APPLY 후 되읽어 확인해야 한다'
    });
  }

  const pick = options.filter(o => o.reproducesNoCaption)[0] || null;
  captionHandling = {
    masterCaptionNodeExists: v0.captionNodeExists,
    masterCaptionVisibleByDefault: v0.captionVisibleByDefault,
    masterCaptionIsOptional: v0.captionIsOptional,
    masterCaptionBasis: v0.captionOptionalBasis,
    captionGeometry: cg,
    captionHandlingOptions: options,
    captionHandlingDecision: pick ? pick.option : null,
    captionHandlingReason: pick
      ? pick.option + ' 를 택한다 — ' + pick.reason
      : 'caption 노드를 찾지 못해 두 방법을 비교할 수 없다',
    allVariantsAgreeOnCaption: master.variants.every(v =>
      v.captionNodeExists === v0.captionNodeExists &&
      v.captionVisibleByDefault === v0.captionVisibleByDefault),
    heightIfCaptionHidden: cg ? cg.heightIfCaptionHidden : null,
    heightIfCaptionVisible: cg ? cg.heightIfCaptionVisible : null
  };
  if (captionHandling.captionHandlingDecision === null) {
    notes.push('caption 처리 방법을 정하지 못했다 — 마스터에서 caption 노드를 못 찾았다');
  }
  if (!captionHandling.allVariantsAgreeOnCaption) {
    notes.push('variant 마다 caption 기본 상태가 다르다 — 카드마다 따로 봐야 한다');
  }
}

/* ---------- 카드별 caption action ---------- */
if (captionHandling && mapping.length) {
  for (const m of mapping) {
    const card = cards.filter(c => c.id === m.cardId)[0];
    const has = !!(card && card.caption);
    m.captionSource = has ? card.caption.characters : null;
    if (has) {
      m.captionAction = 'set';
      m.captionValue = card.caption.characters;
      m.captionMustBecomeVisible = captionHandling.masterCaptionVisibleByDefault === false;
      m.captionActionNote = m.captionMustBecomeVisible
        ? '마스터 caption 이 기본 숨김이므로 텍스트를 넣고 visible = true 로도 켜야 한다'
        : '마스터 caption 이 기본 노출이라 텍스트만 넣으면 된다';
    } else {
      m.captionAction = captionHandling.captionHandlingDecision === 'B' ? 'hide' : 'empty';
      m.captionValue = m.captionAction === 'hide' ? null : '';
      m.captionMustBecomeVisible = false;
      m.captionActionNote = m.captionAction === 'hide'
        ? (captionHandling.masterCaptionVisibleByDefault === false
            ? '마스터가 이미 숨김이라 그대로 두면 된다'
            : 'visible = false 로 꺼야 한다')
        : '빈 문자열로 두지만 줄 높이는 그대로 남는다';
    }
    /* caption 처리 후 이 카드의 예상 높이 */
    m.predictedHeightAfterContentOverrides = has
      ? captionHandling.heightIfCaptionVisible
      : captionHandling.heightIfCaptionHidden;
    m.captionResolved = has
      ? (m.captionAction === 'set' && typeof m.captionValue === 'string')
      : (m.captionAction === 'hide' || m.captionAction === 'empty');
  }

  const heights = mapping.map(m => m.predictedHeightAfterContentOverrides);
  const allSame = heights.every(h => h !== null && Math.abs(h - heights[0]) < 0.5);
  captionHandling.perCardPredictedHeights = mapping.map(m => ({
    title: m.title, caption: m.caption, action: m.captionAction,
    predictedHeight: m.predictedHeightAfterContentOverrides }));
  captionHandling.allCardsSameHeightAfterOverrides = allSame;
  captionHandling.tallestCardAfterOverrides = heights.length
    ? r2(Math.max.apply(null, heights.filter(h => h !== null))) : null;
  captionHandling.heightSpread = allSame ? 0
    : r2(Math.max.apply(null, heights) - Math.min.apply(null, heights));
  captionHandling.heightNote = allSame
    ? '네 장 모두 ' + heights[0] + ' 으로 같다'
    : 'caption 이 있는 카드와 없는 카드의 높이가 ' + captionHandling.heightSpread +
      'px 다르다. strip 의 counterAxisAlignItems 가 MIN 이면 카드들이 위로 정렬되고 ' +
      '아래쪽이 들쭉날쭉해진다 — 교체 전에 결정이 필요하다.';
  /* 높이가 갈리면 strip 에 무엇이 생기는지 구체적으로 낸다 */
  const align = 'counterAxisAlignItems' in strip ? strip.counterAxisAlignItems : null;
  const padV = r2((strip.paddingTop || 0) + (strip.paddingBottom || 0));
  const tallest = captionHandling.tallestCardAfterOverrides;
  captionHandling.stripConsequence = {
    stripCounterAxisAlignItems: align,
    stripPaddingV: padV,
    stripHeightNow: r2(strip.height),
    tallestCardAfterOverrides: tallest,
    predictedStripHeight: tallest === null ? null : r2(tallest + padV),
    stripHeightDelta: tallest === null ? null : r2(tallest + padV - strip.height),
    allCardsSameHeight: allSame,
    bottomAlignmentDiffers: !allSame && align !== 'MAX',
    cardsStretchToTallest: align === 'STRETCH',
    consequence: allSame
      ? '네 장 높이가 같아 정렬 문제가 없다'
      : (align === 'STRETCH'
          ? 'counterAxisAlignItems 가 STRETCH 라 짧은 카드가 가장 높은 카드에 맞춰 늘어난다 — ' +
            '높이는 맞지만 내용이 위로 몰린다'
          : (align === 'MAX'
              ? 'MAX 라 카드들이 아래로 정렬된다 — 위쪽 제목 줄이 어긋난다'
              : '(' + align + ') 라 카드들이 위로 정렬되고 **아래쪽이 ' +
                captionHandling.heightSpread + 'px 들쭉날쭉해진다.** ' +
                '배지 아래 선이 카드마다 달라 보인다'))
  };
  if (!allSame) {
    notes.push('⚠ caption 처리 후 카드 높이가 갈린다: ' +
      JSON.stringify(captionHandling.perCardPredictedHeights.map(x => x.title + '=' + x.predictedHeight)) +
      '. ' + captionHandling.stripConsequence.consequence +
      ' strip 높이는 ' + r2(strip.height) + ' → ' + captionHandling.stripConsequence.predictedStripHeight +
      '. 폭을 억지로 맞추듯 높이를 억지로 맞추지 말고, caption 을 네 장 모두 쓸지 / 모두 안 쓸지 / ' +
      '이대로 둘지 먼저 정해야 한다.');
  } else {
    notes.push('caption 처리 후 네 장 모두 높이 ' + captionHandling.perCardPredictedHeights[0].predictedHeight +
      ' 로 같다.');
  }
}
...
const gate = {
  ...
  /* caption 은 "없어도 된다" 로 끝나지 않는다. caption 없는 카드를 **어떻게 표현할지** 까지 정해져야 한다. */
  captionHandlingValid: !!(captionHandling &&
    captionHandling.captionHandlingDecision !== null &&
    captionHandling.allVariantsAgreeOnCaption === true &&
    mapping.length === cards.length && cards.length > 0 &&
    mapping.every(m => m.captionResolved === true &&
      (captionHandling.captionHandlingOptions
        .filter(o => o.option === captionHandling.captionHandlingDecision)[0] || {})
        .safeAsInstanceOverride === true)),
  allCaptionMappingsCorrect: mapping.length === cards.length && cards.length > 0 &&
    mapping.every(function (m) {
      const card = cards.filter(c => c.id === m.cardId)[0];
      const expected = card && card.caption ? card.caption.characters : null;
      return m.caption === expected && m.captionSource === expected;
    }),
  cardHeightsPredictable: !!(captionHandling &&
    captionHandling.perCardPredictedHeights &&
    captionHandling.perCardPredictedHeights.every(x => x.predictedHeight !== null)),
...
return out({
  ...
  mapping,
  captionActions: mapping.map(m => ({
    title: m.title,
    captionSource: m.captionSource === undefined ? null : m.captionSource,
    captionAction: m.captionAction === undefined ? null : m.captionAction,
    captionValue: m.captionValue === undefined ? null : m.captionValue,
    captionMustBecomeVisible: m.captionMustBecomeVisible === undefined ? null : m.captionMustBecomeVisible,
    predictedHeightAfterContentOverrides: m.predictedHeightAfterContentOverrides === undefined
      ? null : m.predictedHeightAfterContentOverrides,
    note: m.captionActionNote === undefined ? null : m.captionActionNote
  })),
  ...
  captionHandling,
  captionNote: 'caption 없는 카드를 어떻게 표현할지까지 정해져야 captionHandlingValid 가 참이 된다. ' +
               '빈 문자열은 줄 높이를 그대로 차지해 "caption 없음" 을 재현하지 못한다.',
