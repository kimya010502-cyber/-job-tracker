/* ============================================================================
 * 줍줍 디자인 시스템 정리 — 스크립트 43 (42 의 재구성판)
 * 상세 화면(지원 기업별 상세 페이지) 구현 v2 — PNG 레이아웃/정보밀도 복원
 *
 * 42(v1) 를 실제로 돌려본 사용자 피드백: "구조적으로 원본 PNG의 장점을 너무 많이 잃었다." 42 는 그대로
 * 보존하고, 이 파일이 새 버전이다. **42 가 만든 컴포넌트/화면은 이 스크립트가 건드리지 않는다** — 전부
 * 새로 만든다(사용자가 재생성을 명시적으로 허용). 42 의 남은 결과물은 파일에 그대로 남아있지만 안 쓴다.
 *
 * v1 → v2 에서 고친 것 (PNG 를 다시 레이아웃 기준으로)
 *   1) Summary bar — 항목 사이 얇은 구분선 추가, padding/gap 축소해서 한 줄에 가깝게 compact 하게
 *   2) Timeline — 카드 사이 gap 에 짧은 세로 연결선(뱃지 중심 x 에 맞춤)을 넣어 "하나의 흐름"으로 보이게
 *   3) current 카드 — 좌측에 진짜 accent 막대(둥근 카드 왼쪽에 겹쳐서 render, z-order 로 처리)를 추가
 *   4) waiting-locked 카드 — subtle 틴트 배경을 버리고 current 와 같은 흰 카드 스타일로, 제목은 muted 아니라
 *      일반 Card title 로(가독성), padding 축소
 *   5) 우측 기업 메모 패널 — 폭을 280→304, padding 을 키우고 메모 본문을 여러 줄로
 *   6) 2단 비율 — 좌 648 · 갭 24 · 우 304 = 976(콘텐츠 폭 24+24 여백 뺀 값) 정확히 일치, 약 66.4% / 31.1%
 *   7) 정보 위계는 v1 과 동일한 순서(상단바 → summary → timeline → memo) 유지, 그대로 좋았음
 *   8) **버그 수정** — v1 은 3단계(최종 임원 면접) 인스턴스에 텍스트를 안 바꿔서 "1차 실무 면접"이 두 번
 *      나왔다. 이번엔 waiting-locked 컴포넌트 안 title/desc TEXT 에 이름을 붙여(`step-title`/`step-desc`)
 *      인스턴스별로 정확히 찾아서 override 한다.
 *   9) 전체 padding/gap 을 20~24 → 16~18 로 줄여 밀도를 높인다.
 *   10) Sidebar/Header 는 그대로 clone 재사용(변경 없음), 화면 전체 비율만 PNG 우선.
 *
 * 재사용(41 discovery 그대로, v1 과 동일)
 *   Button 1029:1997 · Select 1030:2007 · Input 1030:2017(leading#1056:12) · Chip 1029:1984
 *   (leading#1052:0) · Icon Button 1037:2091(icon#1055:6) · Icon: ChevronLeft 1048:810 ·
 *   ExternalLink 1048:794 · Calendar 1048:802 · Dot 1048:816
 *
 * 신규(전부 새로 만든다 — 42 의 결과물 재사용 안 함)
 *   Icon / Edit·Trash·Lock·Check·Copy · Timeline Step Card v2(state=current/waiting-locked) ·
 *   Guide Card v2 · 상세 화면 v2 프레임
 *
 * 배치   새 컴포넌트: Chip 세트와 같은 부모 밑, chipSet 아래쪽으로 42 결과물과 안 겹치게 더 내려서.
 *        새 화면 프레임: mainFrame 과 같은 부모 밑, mainFrame 보다 더 아래(y + height*2 + 800)에 —
 *        기존 backup 들 · 42 의 상세 화면과도 안 겹치는 자리.
 *
 * 실행법
 *   1) DRY_RUN = true → 필요한 기존 id 확인 + 배치 좌표만. mutation 0.
 *   2) 같은 scriptVersion 에서 DRY_RUN = false 로 APPLY.
 *   실패 시: 이번에 새로 만든 최상위 노드만 지우고 원상복귀.
 * ========================================================================== */

const DRY_RUN = true;          // ← APPLY 할 때만 false 로 변경
const SCRIPT_VERSION = '43-v1-detail-screen-build-v2';
...
/* 42(v1) 결과물과 안 겹치게 더 아래로 — v1 은 mainFrame.y+height+400 부터 썼다 */
const newScreenX = blockers.length === 0 ? r2(mainFrame.x) : null;
const newScreenY = blockers.length === 0 ? r2(mainFrame.y + mainFrame.height * 2 + 800) : null;
...
try {
  ...
  /* 42(v1) 이 chipSet 아래 ~560px 까지 썼다 — 안 겹치게 700 부터 시작 */
  let iy = N.chipSet.y + N.chipSet.height + 700;
...
/* 본문 콘텐츠 폭 976(=Main 1024 − 좌우 padding 24×2), gap 24 를 빼면 카드+메모 합쳐 952.
   PNG 비율(약 68/32)에 맞춰 648 · 304 로 나눈다 — 648+24+304 = 976, 딱 맞음(넘치지 않음). */
const CARD_W = 648, MEMO_W = 304, BADGE_D = 26, ACCENT_W = 4;
try {
  /* -- state=current -- 흰 카드 + 좌측 accent 막대(z-order 로 카드 아래 깔아 카드 왼쪽에 살짝 삐져나오게) -- */
  const curContent = vframe('content', { gap: 14, padX: 18, padY: 18, fixedW: CARD_W - ACCENT_W });
  applyCardStyle(curContent);
  ...
  const badgeWrap = overlayFrame('badge-wrap', BADGE_D, BADGE_D);
  const badge = figma.createEllipse(); badge.name = 'step-badge'; badge.resize(BADGE_D, BADGE_D); badge.x = 0; badge.y = 0; badge.fills = fillFromVar(null, '#4f46e5');
  badgeWrap.appendChild(badge);
  const badgeNum = await mkText('1', 'Label'); badgeNum.fills = fillFromVar(null, '#ffffff'); badgeNum.textAlignHorizontal = 'CENTER'; badgeNum.resize(BADGE_D, badgeNum.height); badgeNum.x = 0; badgeNum.y = 5; badgeWrap.appendChild(badgeNum);
  ...
  curContent.appendChild(curHeaderRow);

  const row1 = vframe('row', { horizontal: true, gap: 14 });
  ...
  curContent.appendChild(row1);
  const row2 = vframe('row', { horizontal: true, gap: 14 });
  ...
  curContent.appendChild(memoHeader);
  const memoBox = vframe('memo-box', { padX: 12, padY: 10, fixedW: CARD_W - ACCENT_W - 36 });
  memoBox.fills = fillFromVar(subtleBgVar, '#eff4ff'); memoBox.cornerRadius = 8;
  memoBox.appendChild(await mkText('서류 제출 완료. 사전 과제 안내는 접수 후 영업일 5일 이내 전달 예정이라고 기재됨. PiaVision 대시보드 리텐션 개선안에 대한 사례 위주로 서류 기술서 보강 완료.', 'Body'));
  curContent.appendChild(memoBox);

  /* accent 막대 — curContent 를 다 만든 뒤에 높이를 재서 정확히 맞춘다(Figma 는 HUG 프레임 높이를
     자식을 다 넣은 시점에 바로 계산해준다). z-order 상 curContent 보다 먼저 넣어 뒤에 깔리게 한다 */
  const accent = figma.createRectangle(); accent.name = 'accent'; accent.resize(ACCENT_W + 2, curContent.height);
  accent.topLeftRadius = 8; accent.bottomLeftRadius = 8; accent.topRightRadius = 0; accent.bottomRightRadius = 0;
  accent.fills = fillFromVar(null, '#6155f5');

  const cur = figma.createComponent(); cur.name = 'state=current'; cur.layoutMode = 'NONE'; cur.fills = [];
  cur.appendChild(accent); accent.x = 0; accent.y = 0;
  cur.appendChild(curContent); curContent.x = ACCENT_W; curContent.y = 0;
  cur.resize(curContent.width + ACCENT_W, curContent.height);

  /* -- state=waiting-locked -- current 와 같은 흰 카드, muted 는 설명 문구에만, 제목은 정상 가독성 -- */
  const locked = figma.createComponent(); locked.name = 'state=waiting-locked'; locked.layoutMode = 'HORIZONTAL'; locked.itemSpacing = 12;
  locked.paddingLeft = locked.paddingRight = 18; locked.paddingTop = locked.paddingBottom = 14;
  locked.primaryAxisAlignItems = 'SPACE_BETWEEN'; locked.counterAxisAlignItems = 'CENTER';
  locked.layoutSizingHorizontal = 'FIXED'; locked.resize(CARD_W, 56);
  applyCardStyle(locked);
  const lockedLeft = vframe('left', { horizontal: true, gap: 10, calign: 'CENTER' });
  const lockedBadgeWrap = overlayFrame('badge-wrap', BADGE_D, BADGE_D);
  const lockedBadge = figma.createEllipse(); lockedBadge.resize(BADGE_D, BADGE_D); lockedBadge.x = 0; lockedBadge.y = 0; lockedBadge.fills = fillFromVar(null, '#c7c4d8'); lockedBadgeWrap.appendChild(lockedBadge);
  ...
  const lockedTitle = await mkText('1차 실무 면접', 'Card title'); lockedTitle.name = 'step-title';
  lockedTitleRow.appendChild(lockedTitle);
  lockedTitleRow.appendChild(await chipInstance('waiting', '대기중'));
  lockedTextCol.appendChild(lockedTitleRow);
  const lockedDesc = await mkText('이전 전형(서류)을 통과하면 활성화됩니다.', 'Caption', { muted: true }); lockedDesc.name = 'step-desc';
  lockedTextCol.appendChild(lockedDesc);
  ...
  timelineCardSet.x = N.chipSet.x; timelineCardSet.y = N.chipSet.y + N.chipSet.height + 900;
  createdTopLevel.push(timelineCardSet.id); mutationCount++;

  /* ======== Guide Card v2 — 폭을 우측 패널(320)에 맞춤, 여유 있게 ======== */
  guideCard = figma.createComponent(); guideCard.name = 'Guide Card'; guideCard.layoutMode = 'VERTICAL'; guideCard.itemSpacing = 6;
  guideCard.paddingLeft = guideCard.paddingRight = guideCard.paddingTop = guideCard.paddingBottom = 16;
  guideCard.layoutSizingHorizontal = 'FIXED'; guideCard.resize(MEMO_W, 90);
  guideCard.cornerRadius = 8; guideCard.fills = fillFromVar(subtleBgVar, '#eff4ff');
  const guideTitleRow = vframe('title-row', { horizontal: true, gap: 6, calign: 'CENTER' });
  guideTitleRow.appendChild(await mkText('줍줍(JOOB) 기록 가이드', 'Label'));
  guideCard.appendChild(guideTitleRow);
  ...
  guideCard.x = N.chipSet.x; guideCard.y = N.chipSet.y + N.chipSet.height + 1200;
  ...
  mainContent.itemSpacing = 14; mainContent.paddingLeft = mainContent.paddingRight = 24; mainContent.paddingTop = 64; mainContent.paddingBottom = 64;
  ...
  /* -- Summary Bar — compact, 항목 사이 얇은 구분선으로 한 줄에 가깝게 -- */
  const summaryBar = vframe('summary-bar', { horizontal: true, gap: 14, padX: 16, padY: 10, calign: 'CENTER', fixedW: mainFrame.width - contentX - 48 });
  applyCardStyle(summaryBar);
  function summaryDivider() { const d = figma.createRectangle(); d.name = 'divider'; d.resize(1, 14); d.fills = fillFromVar(null, '#e5e3f0'); return d; }
  ...
  summaryBar.appendChild(summaryDivider());
  ...
  /* 카드 사이 짧은 연결선 — 뱃지 중심 x(대략 33) 에 맞춰 흐름이 이어지는 것처럼 보이게 한다.
     정확한 픽셀 정합은 실제로 보면서 조정이 필요할 수 있다는 걸 알아둔다. */
  function connector(h) {
    const wrap = overlayFrame('connector', CARD_W, h);
    const line = figma.createRectangle(); line.name = 'line'; line.resize(2, h); line.x = 32; line.y = 0; line.fills = fillFromVar(null, '#d8d5ea');
    wrap.appendChild(line);
    return wrap;
  }
  /* waiting-locked 인스턴스 안에서 이름 붙여둔 텍스트를 재귀로 찾아 문구만 override */
  function findByName(node, name) {
    for (const c of kids(node)) { if (c.name === name) return c; const found = findByName(c, name); if (found) return found; }
    return null;
  }
  async function overrideLockedText(inst, title, desc) {
    const t = findByName(inst, 'step-title'); if (t) { await loadFontSafe(t.fontName); t.characters = title; }
    const d = findByName(inst, 'step-desc'); if (d) { await loadFontSafe(d.fontName); d.characters = desc; }
  }

  leftCol.itemSpacing = 0;
  leftCol.appendChild(curVariant.createInstance());
  leftCol.appendChild(connector(10));
  const step2 = lockedVariant.createInstance();
  await overrideLockedText(step2, '1차 실무 면접', '이전 전형(서류)을 통과하면 활성화됩니다. 실무진 직무 인터뷰 및 라이브 과제 리뷰 준비 예정');
  leftCol.appendChild(step2);
  leftCol.appendChild(connector(10));
  const step3 = lockedVariant.createInstance();
  await overrideLockedText(step3, '최종 임원 면접', '컬처핏 및 처우 협의 단계');
  leftCol.appendChild(step3);
  body.appendChild(leftCol);

  /* -- 우측 기업 메모 패널 v2 — 320 폭, 여백/정보 밀도 확대 -- */
  const rightCol = vframe('memo-panel', { gap: 16, fixedW: MEMO_W });
  const memoPanel = vframe('memo-card', { gap: 12, padX: 18, padY: 18, fixedW: MEMO_W });
  ...
  const memoDetailHeader = vframe('detail-header', { horizontal: true, gap: 8, align: 'SPACE_BETWEEN', calign: 'CENTER' });
  memoDetailHeader.appendChild(await mkText('기업 상세 메모', 'Label'));
  memoDetailHeader.appendChild(await mkText('마지막 수정: 3시간 전', 'Caption', { muted: true }));
  memoPanel.appendChild(memoDetailHeader);
  memoPanel.appendChild(await mkText('국내 주요 이커머스 솔루션 및 풀필먼트 B2B SaaS 제공 기업', 'Body'));
  memoPanel.appendChild(await mkText('주력 서비스의 판매자 콘솔 UI 및 데이터 대시보드 리뉴얼 추진 중', 'Body'));
  memoPanel.appendChild(await mkText('협업 도구: Slack, Notion, Jira 기반 애자일 스프린트 운영', 'Body'));
  memoPanel.appendChild(await mkText('직무 핵심: 백오피스 관리자 콘솔 기획 및 파트너 정산 플로우 개선', 'Body'));
