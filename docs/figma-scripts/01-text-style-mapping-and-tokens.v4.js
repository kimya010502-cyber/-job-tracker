 ...
 *
 * v3 변경점 (v2 DRY_RUN 결과 반영)
 *   - #f8f9ff 를 hex만으로 surface/header 에 묶던 것을 opacity까지 확인하도록 수정.
 *     불투명 #f8f9ff(페이지 배경)는 80% 토큰에 묶으면 배경이 비쳐 보이므로 바인딩하지
 *     않고 unmappedNotable 로 보고만 한다.
 *
 * v4 변경점 (v3 DRY_RUN 결과 반영)
 *   - 불투명 #f8f9ff 를 예외로 남기지 않고 surface/page 토큰을 신규 생성해 바인딩한다.
 *     같은 hex라도 역할과 opacity가 다르므로 별도 semantic token으로 관리한다.
 *       surface/page   #f8f9ff 100%  ← 본문 페이지 배경 (1002:3 Main)
 *       surface/header #f8f9ff  80%  ← 상단바 배경 (유지)
 *
 * 실행법
...
// surface/page (본문 페이지 배경). surface/header 와 같은 hex지만 opacity와 역할이 다르다.
if (!V['surface/page']) {
  variablesWouldCreate.push('surface/page');
  if (!DRY_RUN) {
    const v = figma.variables.createVariable('surface/page', colorCol, 'COLOR');
    v.setValueForMode(colorCol.modes[0].modeId, { r: 248 / 255, g: 249 / 255, b: 255 / 255, a: 1 });
    v.scopes = ['FRAME_FILL', 'SHAPE_FILL'];
    V['surface/page'] = v;
    variablesCreated.push('surface/page');
  }
}
...
for (const n of all) {
  try {
    ...
    if (NODE_OVERRIDE[n.id]) {
      ...
      // 같은 #f8f9ff 라도 80%면 상단바(surface/header), 불투명이면 페이지 배경(surface/page).
      const o8 = opacityOf(p);
      token = (Math.abs(o8 - 0.8) <= 0.05) ? 'surface/header' : 'surface/page';
      via = 'opacity-rule';
    } else if (h === '#ffffff') {
