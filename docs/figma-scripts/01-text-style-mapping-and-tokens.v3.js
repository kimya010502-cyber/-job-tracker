...
for (const n of all) {
  try {
    ...
    if (NODE_OVERRIDE[n.id]) {
      ...
      // surface/header 는 80% 토큰이다. 불투명한 같은 색(페이지 배경)을 여기 묶으면 배경이 비친다.
      const o8 = opacityOf(p);
      if (Math.abs(o8 - 0.8) <= 0.05) { token = 'surface/header'; via = 'hex-map'; }
      else {
        unmappedNotable.push({ id: n.id, name: n.name, type: n.type, hex: h,
          opacity: Math.round(o8 * 1000) / 1000, reason: 'opaque-page-surface / surface-header는 80% 토큰이라 제외' });
        continue;
      }
    } else if (h === '#ffffff') {
