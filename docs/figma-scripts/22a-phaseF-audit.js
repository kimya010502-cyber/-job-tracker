 ...
 * Phase F AUDIT · Sidebar NavItem 6개 + Pagination Item 4개 (읽기 전용)
 *
...
const SCRIPT_VERSION = '22a-v1-phaseF-audit';
...
const NAV_MEANING = [
  { key: 'applications', label: /지원|현황|대시보드|홈|목록|application|dashboard|home/i,
...
for (let i = 0; i < navTargets.length; i++) {
  ...
  /* 배경 칠 — 자기 자신, 없으면 첫 번째 칠 있는 자식 프레임 */
  ...
  navRows.push({
    ...
    originalIndex: (kids(navParent.node) || []).indexOf(n),
...
/* active 판정 — 배경 칠 서명이 혼자 다른 항목. 안 되면 글자색, 그다음 글자 굵기. */
...
for (const r of navRows) {
  ...
  r.plannedIcon = key ? iconLibrary[key].name : null;
  r.plannedIconId = key ? iconLibrary[key].id : null;
...
if (duplicateNavIcons.length) notes.push('같은 아이콘이 두 항목 이상에 매핑됐다: ' + duplicateNavIcons.join(', ') + ' — 매핑을 다시 봐야 한다');
...
for (const r of navRows) {
  ...
  const risks = [];
  if (mw !== null && !near(r.geometry.width, mw)) {
    risks.push('폭 ' + r.geometry.width + ' → ' + mw + ' (old 가로 sizing: ' + r.layout.layoutSizingHorizontal + ')');
  }
  if (mh !== null && !near(r.geometry.height, mh)) risks.push('높이 ' + r.geometry.height + ' → ' + mh);
  if (r.textNodeCount !== 1) risks.push('TEXT 가 ' + r.textNodeCount + '개 — 라벨을 하나로 확정하기 어렵다');
  if (!r.oldIcon) risks.push('기존 아이콘을 찾지 못했다');
  else if (r.oldIcon.candidateCount > 1) risks.push('아이콘 후보가 ' + r.oldIcon.candidateCount + '개');
...
const navParentPrediction = navNew ? predictParent(navParent, navRows, navNew.width, navNew.height) : null;
...
if (navParent && navNew) {
  ...
  if (navParentPrediction && navParentPrediction.mainHug &&
      !near(navParentPrediction.predictedMainAfter, navParent.geometry.height)) {
    risks.nav.push('Nav 부모 높이가 ' + navParent.geometry.height + ' → ' + navParentPrediction.predictedMainAfter + ' 로 바뀐다');
  }
...
/* ---------- gate ---------- */
const navHidden = hiddenStrategyOf(navParent);
const pagHidden = hiddenStrategyOf(pagParent);
const dupIds = list => list.length !== new Set(list).size;
const navIdx = navRows.map(r => r.originalIndex), pagIdx = pagRows.map(r => r.originalIndex);

const gate = {
  navParentFound: !!navParent,
  exactly6NavTargets: navRows.length === 6,
  ...
  allNavLabelsResolved: navRows.length === 6 && navRows.every(r => !!r.label && r.textNodeCount === 1),
  allNavStatesResolved: navRows.length === 6 && navRows.every(r => !!r.oldState),
  allNavIconMappingsResolved: navRows.length === 6 && navRows.every(r => !!r.plannedIconId) &&
  ...
  replacementOrderMeasurable: navIdx.every(i => i >= 0) && pagIdx.every(i => i >= 0) &&
    !dupIds(navIdx) && !dupIds(pagIdx),
  parentGeometryMeasurable: !!(navParentPrediction && navParentPrediction.measurable &&
    pagParentPrediction && pagParentPrediction.measurable),
  navHiddenStrategyApplicable: navHidden.applicable === true,
...
for (const r of navRows) if (r.iconMappingConfidence !== 'high') {
  notes.push(r.key + ' "' + r.label + '" 아이콘 매핑 신뢰도 ' + r.iconMappingConfidence + ' — ' + r.iconMappingSource);
}
...
const navTable = navRows.map(r => ({
  key: r.key, srcId: r.srcId, label: r.label,
...
return out({
  ...
  paginationPrevNextIconColorsDiffer: prevNextColorDiffer,

  sidebarNavParentBefore: navParent ? { id: navParent.id, name: navParent.name, geometry: navParent.geometry,
    layout: navParent.layout, childCount: navParent.childCount, hiddenChildIds: navParent.hiddenChildIds,
    absoluteChildIds: navParent.absoluteChildIds, parentOfParent: navParent.parentOfParent } : null,
  paginationParentBefore: pagParent ? { id: pagParent.id, name: pagParent.name, geometry: pagParent.geometry,
    layout: pagParent.layout, childCount: pagParent.childCount, hiddenChildIds: pagParent.hiddenChildIds,
    absoluteChildIds: pagParent.absoluteChildIds, parentOfParent: pagParent.parentOfParent } : null,

  expectedAfter: {
    navVisibleChildCount: navRows.length, paginationVisibleChildCount: pagRows.length,
    navParentSize: navParentPrediction ? navParentPrediction.predictedSizeAfter : null,
    paginationParentSize: pagParentPrediction ? pagParentPrediction.predictedSizeAfter : null,
    note: 'old 를 숨기는 방식이면 부모의 전체 자식 수는 늘지만 보이는 자식 수는 그대로다'
  },
  parentPrediction: { nav: navParentPrediction, pagination: pagParentPrediction },
  risks,
  hiddenChildStrategy: { nav: navHidden, pagination: pagHidden },
  ...
  nextStep: gatePassed
    ? 'gate 전부 통과. 매핑 표를 확인받은 뒤 22-v1 DRY_RUN 을 씁니다.'
