 ...
 * v2: 30-v2 기준값을 읽는다. 인스턴스 local 위치(4,0)와 Margin 구조(VERTICAL · 0/0/0/4)를 직접 확인하고,
 *     초기화→토글 간격은 local 계산과 absoluteTransform 두 가지로 재서 둘 다 12 여야 통과한다.
 * ========================================================================== */

const SCRIPT_VERSION = '30b-G1B-v2-viewtoggle-replace-verify';
...
else if (baseline.scriptVersion !== '30-G1B-v2-viewtoggle-replace') notes.push('기준값 scriptVersion 이 예상과 다르다: ' + baseline.scriptVersion);
...
c.instanceLocalPos4x0 = !!inst && near(inst.x, 4, 0.01) && near(inst.y, 0, 0.01);
c.marginLayoutStillVertical = !!mg && sg(mg, 'layoutMode') === 'VERTICAL' && pads(mg).t === 0 && pads(mg).r === 0 && pads(mg).b === 0 && pads(mg).l === 4;
...
const prevBody = prev ? flow(prev).reduce((best, x) => (!best || x.x + x.width > best.x + best.width ? x : best), null) : null;
const gapAbsolute = prevBody && inst ? r2(inst.absoluteTransform[0][2] - (prevBody.absoluteTransform[0][2] + prevBody.width)) : null;
c.resetToggleGapPreserved = near(gap, EXPECT.gap, 0.01) && near(gapAbsolute, EXPECT.gap, 0.01);
...
return out({
  ...
  measured: {
    ...
    toggleCenterY: centerY, toolbarCenterY: tb ? r2(tb.height / 2) : null, resetToToggleGap: gap, resetToToggleGapAbsolute: gapAbsolute
  },
