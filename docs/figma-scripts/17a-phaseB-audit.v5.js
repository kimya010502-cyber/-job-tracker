...
if (bodyNode) {
  ...
  /* generic scanner 에 안 잡혀도 구조 resolver 가 근거를 갖고 확정했으면 coverage 는 확보된 것이다.
   * ghost 버튼이 모양 스캔에 안 걸리는 건 결함이 아니라 그 버튼의 성질이다. */
  resetRec.independentlyResolvedByStructure = true;
  resetRec.coverageBasis = resetRec.scannedAsControl
    ? 'generic scanner + structure resolver'
    : 'structure resolver 단독 (ghost 라 모양 스캔에는 안 걸림 — 정상)';
...
for (const c of controls) {
  ...
  let cls = null;
  /* 이 스크립트가 다른 곳에서 이미 역할을 밝힌 노드를 여기서 "정체 불명" 이라 하면 안 된다.
   * 한 스크립트 안에서 같은 노드에 두 가지 결론을 내는 것이 진짜 오류다. */
  const rr = resetRoles.filter(r => r.id === c.id && r.exists &&
                                    r.roleVerdict && r.roleVerdict !== 'unknown')[0];
  const isResetWrapper = resetWrapperStrategy && resetWrapperStrategy.wrapperId === c.id;
  const wrapsKnownTarget = targets.filter(t => t.self &&
    (t.ancestorIds || []).indexOf(c.id) >= 0)[0];

  if (rr) {
    cls = { rule: 'resetRoles', label: 'Reset ' +
      (rr.roleVerdict.indexOf('A.') === 0 ? 'margin wrapper'
        : rr.roleVerdict.indexOf('C.') === 0 ? 'label container' : 'button body') +
      ' — ' + rr.roleVerdict };
  } else if (isResetWrapper) {
    cls = { rule: 'resetWrapperStrategy', label: 'Reset margin wrapper' };
  } else if (wrapsKnownTarget) {
    cls = { rule: 'wrapsKnownTarget',
            label: wrapsKnownTarget.key + ' 를 감싸는 wrapper — 교체 대상이 아니라 그 부모다' };
  } else if (/view|toggle|보기/i.test(c.name + ' ' + texts.join(' '))) {
...
/* ---------- 후보: Input wrapper 폭을 새 Input 에 맞추기 ----------
 * 목적은 "9px 확보" 가 아니라 **wrapper sizing 을 새 Input 구조와 일치시키는 것** 이다.
 * wrapper 가 249 FIXED 인데 안에 들어갈 in-flow 내용이 240 하나뿐이면 9px 은 죽은 여백이다.
 * 여기서는 계산만 한다. 폭을 바꾸지 않는다. */
const inputTarget = targets.filter(t => t.key === 'input')[0];
let wrapperShrink = null;

if (inputTarget && inputTarget.self) {
  const inputNode = await figma.getNodeByIdAsync(inputTarget.expectedId);
  const wrap = inputNode ? inputNode.parent : null;
  const newInputWidth = (widthPlan.filter(w => w.key === 'input')[0] || {}).predictedWidth;

  if (wrap && newInputWidth !== null && newInputWidth !== undefined) {
    const ch = kids(wrap) || [];
    const inFlow = ch.filter(c => c.layoutPositioning !== 'ABSOLUTE');
    const abs = ch.filter(c => c.layoutPositioning === 'ABSOLUTE');
    const padH = r2((wrap.paddingLeft || 0) + (wrap.paddingRight || 0));
    const gap = r2(wrap.itemSpacing || 0);

    /* 교체 후 이 wrapper 안에서 **레이아웃에 참여하는** 것은 새 Input 하나뿐이다.
     * 기존 본체는 숨김, 기존 검색 아이콘은 ABSOLUTE 이면서 숨김이 된다. */
    const hiddenAfter = [inputTarget.expectedId].concat(
      accessories.filter(a => a.hostKey === 'input' && a.roleConfident && a.duplicateRiskIfKept)
                 .map(a => a.id));
    const inFlowAfter = inFlow.filter(c => hiddenAfter.indexOf(c.id) < 0);
    const contentAfterShrink = r2(newInputWidth +
      inFlowAfter.reduce((a, c) => a + c.width, 0) +
      gap * Math.max(0, inFlowAfter.length + 1 - 1) + padH);

    wrapperShrink = {
      wrapperId: wrap.id, wrapperName: wrap.name,
      expectedId: '1003:1696',
      wrapperMatchesExpected: wrap.id === '1003:1696',
      isToolbarDirectChild: (kids(toolbar) || []).some(g => g.id === wrap.id),

      layoutMode: 'layoutMode' in wrap ? wrap.layoutMode : null,
      currentWidth: r2(wrap.width), currentHeight: r2(wrap.height),
      layoutSizingHorizontal: (function () { try { return wrap.layoutSizingHorizontal; } catch (e) { return null; } })(),
      layoutSizingVertical: (function () { try { return wrap.layoutSizingVertical; } catch (e) { return null; } })(),
      primaryAxisSizingMode: 'primaryAxisSizingMode' in wrap ? wrap.primaryAxisSizingMode : null,
      counterAxisSizingMode: 'counterAxisSizingMode' in wrap ? wrap.counterAxisSizingMode : null,
      padding: 'paddingTop' in wrap
        ? [wrap.paddingTop, wrap.paddingRight, wrap.paddingBottom, wrap.paddingLeft].map(r2).join('/') : null,
      paddingH: padH, gap,
      clipsContent: 'clipsContent' in wrap ? wrap.clipsContent : null,
      minWidth: 'minWidth' in wrap ? wrap.minWidth : null,
      maxWidth: 'maxWidth' in wrap ? wrap.maxWidth : null,

      inFlowChildren: inFlow.map(c => ({ id: c.id, name: c.name, width: r2(c.width),
        visible: c.visible,
        ...
        constraintsH: c.constraints ? c.constraints.horizontal : null })),
      absoluteChildren: abs.map(c => ({ id: c.id, name: c.name, width: r2(c.width),
        x: r2(c.x), rightEdge: r2(c.x + c.width), visible: c.visible,
        constraintsH: c.constraints ? c.constraints.horizontal : null,
        willBeHidden: hiddenAfter.indexOf(c.id) >= 0 })),
      hiddenAfterReplacement: hiddenAfter,

      newInputWidth,
      inFlowCountAfterReplacement: inFlowAfter.length + 1,
      contentWidthAfterReplacement: contentAfterShrink
    };

    /* ---- 안전성 ---- */
    const candidateWidth = contentAfterShrink;
    wrapperShrink.candidateWidthA = candidateWidth;
    wrapperShrink.overflowAfterShrink = r2(contentAfterShrink - candidateWidth);
    wrapperShrink.inputFitsWithoutOverflow = contentAfterShrink <= candidateWidth + 0.5;

    /* 남아 있는(보이는) ABSOLUTE 자식이 새 폭 밖으로 나가는지 */
    const absVisibleAfter = abs.filter(c => hiddenAfter.indexOf(c.id) < 0 && c.visible !== false);
    wrapperShrink.absoluteChildrenStillVisible = absVisibleAfter.map(c => ({
      id: c.id, rightEdge: r2(c.x + c.width) }));
    wrapperShrink.absoluteOverflow = absVisibleAfter.filter(c => (c.x + c.width) > candidateWidth + 0.5)
      .map(c => c.id);
    wrapperShrink.absoluteClippingRisk = wrapperShrink.absoluteOverflow.length > 0 &&
                                         wrap.clipsContent === true;

    /* 폭이 바뀌면 같이 늘어나거나 줄어드는 자식이 있는지 (STRETCH / SCALE 제약) */
    wrapperShrink.childrenAffectedByParentResize = ch.filter(c =>
      c.constraints && (c.constraints.horizontal === 'STRETCH' || c.constraints.horizontal === 'SCALE'))
      .map(c => ({ id: c.id, name: c.name, constraintsH: c.constraints.horizontal,
                   visible: c.visible,
                   note: c.visible === false
                     ? '숨김 상태라 화면에는 영향 없지만, 되돌릴 때 원래 폭이 아닐 수 있다'
                     : '보이는 자식이라 폭이 같이 바뀐다 — 확인 필요' }));

    /* 형제 영향: 이 wrapper 가 툴바 직계면 다른 형제의 폭은 변하지 않는다 (SPACE_BETWEEN 은 간격만 조정) */
    wrapperShrink.siblingImpact = wrapperShrink.isToolbarDirectChild
      ? '툴바가 SPACE_BETWEEN 이라 형제 폭은 변하지 않고 간격만 넓어진다'
      : '툴바 직계가 아니다 — 상위 그룹 폭도 같이 확인해야 한다';

    /* ---- 전략 A / B ---- */
    const hugWidth = contentAfterShrink;
    wrapperShrink.optionA = {
      what: 'layoutSizingHorizontal 을 FIXED 로 두고 width 를 ' + candidateWidth + ' 로 변경',
      predictedWidth: candidateWidth,
      change: '속성 1개 (width)',
      pros: '변경량이 가장 적다. 현재 sizing 모드를 그대로 둔다',
      cons: '내용이 나중에 바뀌면 다시 어긋난다. 죽은 여백이 생기는 구조 자체는 그대로다',
      rollback: '원본을 다시 켜면 wrapper 는 여전히 ' + candidateWidth + ' 라 249 짜리 원본이 넘친다'
    };
    wrapperShrink.optionB = {
      what: 'layoutSizingHorizontal 을 HUG 로 변경 (내용 폭을 따라가게)',
      predictedWidth: hugWidth,
      change: '속성 1개 (sizing mode) — 폭은 계산으로 따라온다',
      pros: '새 Input 구조와 wrapper sizing 이 일치한다. 내용이 바뀌면 알아서 맞춰진다. ' +
            '원본을 되살릴 때도 wrapper 가 249 로 알아서 돌아간다',
      cons: 'sizing 모드를 바꾸는 것은 width 를 바꾸는 것보다 구조 변경이다. ' +
            '안에 FILL(layoutGrow=1) 자식이 있으면 HUG 와 충돌한다',
      fillChildConflict: inFlow.filter(c => c.layoutGrow === 1).map(c => c.id),
      rollback: 'wrapper 가 내용을 따라가므로 원본을 되살리면 자동으로 249 로 복귀'
    };
    const noFillConflict = wrapperShrink.optionB.fillChildConflict.length === 0;
    wrapperShrink.recommended = noFillConflict ? 'B' : 'A';
    wrapperShrink.recommendationBasis = noFillConflict
      ? '목적이 "9px 확보" 가 아니라 "wrapper sizing 을 내용과 일치시키기" 이므로 HUG 가 목적에 맞는다. ' +
        '되돌릴 때도 자동으로 복구된다. FILL 자식이 없어 충돌 위험도 없다.'
      : 'FILL(layoutGrow=1) 자식이 있어 HUG 와 충돌한다 — A 를 쓴다';
    wrapperShrink.recoveredWidth = r2(wrap.width - candidateWidth);
  }
}

/* ---------- 후보를 적용했을 때 툴바 폭 재계산 ---------- */
let candidateImpact = null;
if (wrapperShrink && wrapperShrink.isToolbarDirectChild) {
  const wid = wrapperShrink.wrapperId;
  const cand = wrapperShrink.candidateWidthA;
  function occupancyWithCandidate(widthKey) {
    const groups = toolbarChildren.filter(g => g.visible !== false);
    return r2(groups.reduce(function (a, g) {
      return a + (g.id === wid ? cand : g[widthKey]);
    }, 0) + tb.gap * Math.max(0, tb.visibleChildCount - 1) + tb.paddingH);
  }
  const afterMed = occupancyWithCandidate('predictedWidth');
  const afterHigh = occupancyWithCandidate('predictedWidthHigh');
  const afterLow = occupancyWithCandidate('predictedWidthLow');
  const shortfall = r2(Math.max(0, -(tb.width - contentAfter)));

  candidateImpact = {
    candidate: 'Input wrapper ' + wid + ' 를 ' + wrapperShrink.currentWidth + ' → ' + cand,
    currentLeftGroupWidth: wrapperShrink.currentWidth,
    candidateLeftGroupWidth: cand,
    recoveredWidth: wrapperShrink.recoveredWidth,
    shortfallBeforeCandidate: shortfall,
    recoveredCoversShortfall: wrapperShrink.recoveredWidth >= shortfall,

    predictedOccupiedWidthBeforeCandidate: contentAfter,
    predictedOccupiedWidthAfterCandidate: afterMed,
    predictedOccupiedWidthAfterCandidateLow: afterLow,
    predictedOccupiedWidthAfterCandidateHigh: afterHigh,

    freeSpaceAfterCandidate: r2(tb.width - afterMed),
    freeSpaceAfterCandidateConservative: r2(tb.width - afterHigh),
    collisionRiskAfterCandidate: r2(tb.width - afterHigh) < 0,
    collisionRiskAfterCandidateMedian: r2(tb.width - afterMed) < 0,

    toolbarWidthUnchanged: true,
    mastersUnchanged: true,
    note: 'collisionRiskAfterCandidate 는 보수적 상한 기준이다. 중앙값 기준은 별도 필드에 있다. ' +
          '툴바 폭 976 과 디자인 시스템 마스터는 건드리지 않는다.'
  };

  candidateImpact.acceptable =
    candidateImpact.recoveredCoversShortfall &&
    candidateImpact.freeSpaceAfterCandidate >= 0 &&
    candidateImpact.collisionRiskAfterCandidate === false &&
    wrapperShrink.inputFitsWithoutOverflow === true &&
    wrapperShrink.absoluteClippingRisk === false;
  candidateImpact.acceptanceChecks = {
    recoveredCoversShortfall: candidateImpact.recoveredCoversShortfall,
    freeSpaceAfterCandidateNonNegative: candidateImpact.freeSpaceAfterCandidate >= 0,
    noCollisionConservative: candidateImpact.collisionRiskAfterCandidate === false,
    inputFitsWithoutOverflow: wrapperShrink.inputFitsWithoutOverflow === true,
    noAbsoluteClipping: wrapperShrink.absoluteClippingRisk === false
  };
  if (!candidateImpact.acceptable) {
    notes.push('후보를 적용해도 조건을 다 만족하지 못한다 — acceptanceChecks 를 볼 것. ' +
               '폭을 더 깎지 말고 다른 대안을 찾아야 한다.');
  }
}
...
const gate = {
  ...
  allKnownTargetsScanned: targets.every(t => !t.found ||
    t.scannedAsControl === true || t.independentlyResolvedByStructure === true),
