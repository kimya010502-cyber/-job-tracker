/* ============================================================================
 * 줍줍 디자인 시스템 정리 — 스크립트 15e
 * Pagination Item 아이콘 슬롯 검증 (읽기 전용)
 *
 ...
 *
 * 대상: Pagination Item 컴포넌트 세트 1042:46 (variant 3)
 *
 * 성공 조건
 *   - icon INSTANCE_SWAP 속성이 존재
 *   - 기본값이 Icon / Chevron Left 를 가리킴 (노드 id 또는 컴포넌트 key — 형태는 단정하지 않는다)
 *   - 3개 variant 모두 icon 이 INSTANCE 이고 16×16 이며 숨김
 *   - 외곽 32×32 · padding 0/0/0/0 · gap 0 유지
 *     (슬롯이 12→16 으로 커지지만 숨김 상태이고 외곽이 고정이라 크기가 변하면 안 된다)
 *
 * 참고만 하는 값 (성공 조건 아님)
 *   - instanceCountRecursive — findAll 이 인스턴스 내부까지 재귀하므로 숫자가 커질 수 있다
 * ========================================================================== */

const SCRIPT_VERSION = '15e-v1-pagination-verify';

const SET_ID = '1042:46';
const SET_NAME = 'Pagination Item';
const PROP_NAME = 'icon';
const DEFAULT_ICON = 'Icon / Chevron Left';
const SLOT = 16;
const EXPECT = { width: 32, height: 32, padding: '0/0/0/0', gap: 0, variantCount: 3, slotVisible: false };
...
const successCriteria = {
  ...
  allSlotsVisibleAsSpec: rows.length > 0 && rows.every(r => r.checks.slotVisibleAsSpec),
...
return out({
  ...
  sizeNote: '슬롯이 12→16 으로 커지지만 숨김이고 외곽 32×32 가 고정이라 크기는 변하지 않아야 한다.',
  successCriteria,
  successCriteriaMet,
  notes,
