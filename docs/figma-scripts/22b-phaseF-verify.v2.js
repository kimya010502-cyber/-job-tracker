 ...
 * v2: footer 세로 — 실제 높이·y·bottom·pagination y 가 22-v2 예측과 같은지, 부모 넘침/잘림이 새로 생겼는지 본다.
 *     Phase E 의 기존 겹침은 실패로 보지 않고, 기준값보다 늘었을 때만 실패로 본다.
 * ========================================================================== */

const SCRIPT_VERSION = '22b-v2-phaseF-verify';
...
/* ---------- footer 세로 — 22-v2 예측과 실제 비교 ---------- */
const footerVertical = { predictionAvailable: false };
const fpPred = baseline && baseline.footerPrediction ? baseline.footerPrediction : null;
if (footer && fpPred && typeof fpPred.heightAfter === 'number') {
  footerVertical.predictionAvailable = true;
  footerVertical.prediction = fpPred;
  const fh = r2(footer.height), fy = r2(footer.y), fb = r2(footer.y + footer.height);
  const py = pagNow ? r2(pagNow.y) : null;
  footerVertical.actual = { height: fh, y: fy, bottom: fb, paginationY: py };
  footerVertical.heightMatches = near(fh, fpPred.heightAfter, 0.5);
  footerVertical.yMatches = near(fy, fpPred.yAfter, 0.5);
  footerVertical.bottomMatches = near(fb, fpPred.bottomAfter, 0.5);
  footerVertical.paginationYMatches = py !== null && near(py, fpPred.paginationYAfter, 0.5);

  const fp = footer.parent;
  if (fp) {
    const overflowNow = r2(Math.max(0, fb - r2(fp.height)));
    footerVertical.parent = { id: fp.id, size: geo(fp).size, clipsContent: safeGet(fp, 'clipsContent') };
    footerVertical.parentOverflowNow = overflowNow;
    footerVertical.parentOverflowBeforeApply = fpPred.parentOverflowBefore;
    footerVertical.newParentOverflowPx = r2(overflowNow - (fpPred.parentOverflowBefore || 0));
    footerVertical.noNewParentOverflow = footerVertical.newParentOverflowPx <= 0.5;
    footerVertical.noNewClipping = !(safeGet(fp, 'clipsContent') === true && footerVertical.newParentOverflowPx > 0.5);

    /* 형제와의 세로 겹침 — 예측(=교체 전과 같아야 함)보다 늘었는지 */
    const fx0 = r2(footer.x), fx1 = r2(footer.x + footer.width);
    footerVertical.siblings = kids(fp).filter(c => c.id !== FOOTER_ID && c.visible !== false).map(c => {
      const hx = r2(c.x) < fx1 - 0.5 && fx0 < r2(c.x + c.width) - 0.5;
      let bottom = r2(c.y + c.height);
      if (c.id === '1002:140' && kids(c).length) {
        bottom = Math.max(bottom, r2(c.y + Math.max.apply(null, kids(c).filter(k => k.visible !== false).map(k => k.y + k.height))));
      }
      /* 22-v2 와 같은 방향 있는 겹침 */
      const above = r2(c.y) < (typeof fpPred.yBefore === 'number' ? fpPred.yBefore : fy) - 0.5;
      const now = !hx ? 0 : (above ? r2(Math.max(0, bottom - fy)) : r2(Math.max(0, fb - r2(c.y))));
      const pred = (fpPred.siblingsOverlapAfter || []).filter(x => x.id === c.id)[0];
      const expected = pred && typeof pred.overlapAfter === 'number' ? pred.overlapAfter : 0;
      return { id: c.id, name: c.name, overlapNow: now, overlapPredicted: expected,
        increaseOverPrediction: r2(now - expected), isKnownPhaseEDebt: c.id === '1002:140' };
    });
    footerVertical.noNewCollision = footerVertical.siblings.every(x => x.increaseOverPrediction <= 0.5);
    footerVertical.existingDebt = footerVertical.siblings.filter(x => x.isKnownPhaseEDebt);
  }
} else {
  footerVertical.reason = !fpPred ? 'APPLY 기준값에 footer 예측이 없다 — 22-v2 로 APPLY 해야 한다' : 'footer 를 찾지 못했다';
  notes.push('footer 세로 검증 불가: ' + footerVertical.reason);
}
...
const successCriteria = {
  ...
  footerPredictionAvailable: footerVertical.predictionAvailable === true,
  footerHeightMatchesPrediction: footerVertical.heightMatches === true,
  footerYMatchesPrediction: footerVertical.yMatches === true,
  footerBottomMatchesPrediction: footerVertical.bottomMatches === true,
  paginationYMatchesPrediction: footerVertical.paginationYMatches === true,
  noNewFooterParentOverflow: footerVertical.noNewParentOverflow === true,
  noNewVerticalClipping: footerVertical.noNewClipping === true,
  noNewVerticalCollision: footerVertical.noNewCollision === true,
...
return out({
  ...
  pagination: pagRows, footer: footerCheck, footerVertical, strayOnPage,
