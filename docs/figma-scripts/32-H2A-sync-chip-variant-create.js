...
const SCRIPT_VERSION = '32-H2A-v1-sync-chip-variant-create';
...
if (DRY_RUN || !preflightPassed) {
  return out({
    ...
    plan: preflightPassed ? [
      ...
      'leading visible → true (Icon / Dot 유지, 속성 참조 유지) · Dot glyph fill → ' + COLORS.dot,
...
async function rollback(reason) {
  const steps = [];
  if (clone && !clone.removed) { try { clone.remove(); steps.push('clone removed'); } catch (e) { steps.push('clone remove failed: ' + e.message); } }
  const after = await takeProtected();
  const instAfter = await chipInstanceSnapshots();
  const variantsBack = kids(set).length === 5 && kids(set).every(c => variants.some(v => v.id === c.id));
  const diffs = [];
  for (const k of Object.keys(protectedBefore.variants)) if (JSON.stringify(protectedBefore.variants[k]) !== JSON.stringify(after.variants[k])) diffs.push('variant ' + k);
  if (JSON.stringify(protectedBefore.iconDot) !== JSON.stringify(after.iconDot)) diffs.push('iconDot');
  if (JSON.stringify(protectedBefore.mainFrame) !== JSON.stringify(after.mainFrame)) diffs.push('mainFrame');
  ...
  const d = set.componentPropertyDefinitions || {};
  const optsBack = toneKey && d[toneKey] ? (d[toneKey].variantOptions || []).slice().sort().join(',') === EXPECTED_TONES.slice().sort().join(',') : false;
  return out({ scriptVersion: SCRIPT_VERSION, mode: 'APPLY', aborted: true, failure: reason, rolledBack: true, rollbackSteps: steps,
    rollbackClean: variantsBack && diffs.length === 0 && instDiff.length === 0 && optsBack,
    variantsBack, toneOptionsBack: optsBack, protectedDiff: diffs, chipInstanceDiff: instDiff.slice(0, 20), setSize: size(set),
    note: '세트 크기를 넓힌 뒤 실패했다면 세트 크기는 되돌리지 않았을 수 있다 — setSize 를 확인', errorCount: 1 });
}
...
c.newVariantLeadingIconDotVisible = !!lead2 && lead2.visible === true && !!lmc2 && lmc2.id === IDS.iconDot && near(lead2.width, 16) && near(lead2.height, 16) &&
  lead2.componentPropertyReferences && lead2.componentPropertyReferences.mainComponent === leadingKey && kids(clone).indexOf(lead2) === srcInfo.leading.index;
c.newVariantDotSuccessStrong = isBound(g2, COLORS.dot);
c.newVariantWidthIsHug = near(clone.width, clone.paddingLeft + clone.paddingRight + lead2.width + clone.itemSpacing + lbl2.width, 0.05);
const after = await takeProtected();
...
return out({
  ...
  toneOptions: opts2, chipSetSize: size(set), chipInstancesChecked: Object.keys(instancesBefore).length,
