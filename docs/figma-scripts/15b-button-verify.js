/* ============================================================================
 * 줍줍 디자인 시스템 정리 — 스크립트 15b
 * Button 아이콘 슬롯 검증 (읽기 전용)
 *
 ...
 *
 * 대상: Button 컴포넌트 세트 1029:1997 (variant 4)
 *
 * 성공 조건
 *   - leading INSTANCE_SWAP 속성이 존재
 *   - 기본값이 Icon / Plus 를 가리킴 (노드 id 또는 컴포넌트 key — 형태는 단정하지 않는다)
 *   - 4개 variant 모두 leading 이 INSTANCE 이고 16×16 이며 숨김
 *   - 외곽 높이 36 · padding 0/12/0/12 · gap 4 유지
 *
 * 참고만 하는 값 (성공 조건 아님)
 *   - instanceCountRecursive — findAll 이 인스턴스 내부까지 재귀하므로 숫자가 커질 수 있다
 * ========================================================================== */

const SCRIPT_VERSION = '15b-v1-button-verify';

const SET_ID = '1029:1997';
const SET_NAME = 'Button';
const PROP_NAME = 'leading';
const DEFAULT_ICON = 'Icon / Plus';
const SLOT = 16;
const EXPECT = { height: 36, padding: '0/12/0/12', gap: 4, variantCount: 4, slotVisible: false };
...
const successCriteria = {
  ...
  geometryIntact: rows.length > 0 && rows.every(r => r.checks.heightOk && r.checks.paddingOk && r.checks.gapOk),
