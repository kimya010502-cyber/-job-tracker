/* ============================================================================
 * 줍줍 디자인 시스템 정리 — 스크립트 34 v2
 * Phase I — Local Text Style 8개(Gothic A1 → Pretendard) fontName 만 교체
 *
 * v1 대비 변경점 (v1 은 보존, 이 파일은 별도)
 *   v1 의 `noExistingNegativeFreeSpace` 가 Main(`1002:3`)의 의도된 vertical scroll(freeH −67,
 *   clipsContent=true · overflowDirection=VERTICAL · sizingV=FIXED)을 overflow 오류로 오판해
 *   DRY_RUN 을 막았다. v2 는 **노드 ID 를 하드코딩하지 않고** clipsContent + overflowDirection +
 *   해당 축 sizing=FIXED 조합으로 "의도된 scroll 컨테이너"를 구조적으로 분류하고:
 *     - scroll 컨테이너: 기존 음수 free space 를 막지 않는다. APPLY 뒤에는 절대값이 아니라
 *       **더 나빠졌는지**(after < before − 0.5)만 비교해 규정하고, 나빠졌더라도 rollback 을
 *       유발하는 실패가 아니라 `scrollRegressions` 로 따로 보고한다(정보용 flag).
 *     - scroll 이 아닌 컨테이너(Toolbar · Search Input 등 일반 고정폭): 기존에 음수면 DRY_RUN 을
 *       막고(`noUnexpectedExistingOverflow`), APPLY 뒤 음수가 되면(절대값 기준 진짜 overflow) 그대로
 *       실패 → rollback. HUG 축 크기 변화 자체는 실패 기준이 아니다 — free space(overflow 여지)만 본다.
 *
 ...
 *           scroll 컨테이너는 별도 분류(위 설명). scroll 이 아닌 컨테이너만 음수 = 진짜 overflow.
 *
 * 실행법
...
const SCRIPT_VERSION = '34-v2-font-pretendard-style-swap';
...
const BASELINE_KEY = 'joob.34.baseline.v2';
const TIGHT_THRESHOLD = 20;      // free space 이 이 미만이면 "주의 필요"로 따로 낸다
const REGRESSION_EPS = 0.5;      // 이 이상 나빠져야 "regression" 으로 본다 (반올림 오차 무시)
...
/* ======== 4. 레이아웃 기준값 (FIXED 축 free space, scroll 컨테이너 구조적으로 분류) ======== */
...
/* clipsContent + overflowDirection + 해당 축 FIXED 조합 = 의도된 scroll. ID 하드코딩 없음 — 어느 컨테이너든 이 구조면 잡힌다. */
...
async function layoutBaseline() {
  ...
  for (const id of containerIds) {
    ...
    rows.push({ id, name: node.name, layoutMode: sg(node, 'layoutMode'), sizingH, sizingV, size: size(node), childCount: f.length, freeW, freeH,
      clipsContent: sc.clipsContent, overflowDirection: sc.overflowDirection, scrollH: sc.scrollH, scrollV: sc.scrollV });
  }
  return rows;
}
...
const tightContainers = layoutBefore.filter(r => (typeof r.freeW === 'number' && !r.scrollH && r.freeW < TIGHT_THRESHOLD) || (typeof r.freeH === 'number' && !r.scrollV && r.freeH < TIGHT_THRESHOLD));
const scrollContainers = layoutBefore.filter(r => r.scrollH || r.scrollV).map(r => ({ id: r.id, name: r.name, axis: r.scrollH ? 'W' : 'H', freeBefore: r.scrollH ? r.freeW : r.freeH }));
gate('layoutBaselineCaptured', layoutBefore.every(r => !r.missing), '레이아웃 기준값 수집 중 못 찾은 노드가 있다');
/* scroll 이 아닌 축만 음수를 "지금 이미 잘못된 상태"로 본다. scroll 축의 음수는 의도된 것이라 막지 않는다. */
const unexpectedNegative = layoutBefore.filter(r =>
  (typeof r.freeW === 'number' && !r.scrollH && r.freeW < -0.5) ||
  (typeof r.freeH === 'number' && !r.scrollV && r.freeH < -0.5)
);
gate('noUnexpectedExistingOverflow', unexpectedNegative.length === 0,
  'scroll 이 아닌 컨테이너인데 지금 이미 overflow 다 (교체와 무관한 기존 문제): ' + unexpectedNegative.map(r => r.id + '(' + r.name + ')').join(','));
...
if (DRY_RUN || !preflightPassed) {
  return out({
    ...
    summary: {
      ...
      layoutBaseline: { containerCount: layoutBefore.length, minFreeSpace, tightContainerCount: tightContainers.length, scrollContainerCount: scrollContainers.length },
      mutationCount, blockerCount: blockers.length
    },
    currentTextStyles: stylesBefore,
    loadResults, mapping,
    scopeImpact,
    layoutContainers: layoutBefore, tightContainers, scrollContainers,
...
/* scroll 이 아닌 컨테이너 = 절대값 기준 새 overflow 면 실패. scroll 컨테이너 = "더 나빠졌는지"만 정보로 flag (실패 아님). */
const layoutAfter = await layoutBaseline();
const newOverflow = [];
const scrollRegressions = [];
for (const before of layoutBefore) {
  const after = layoutAfter.find(a => a.id === before.id);
  if (!after) continue;
  if (typeof before.freeW === 'number' && typeof after.freeW === 'number') {
    if (before.scrollH) { if (after.freeW < before.freeW - REGRESSION_EPS) scrollRegressions.push({ id: before.id, name: before.name, axis: 'W', before: before.freeW, after: after.freeW, delta: r2(after.freeW - before.freeW) }); }
    else if (after.freeW < -0.5) newOverflow.push({ id: before.id, name: before.name, axis: 'W', before: before.freeW, after: after.freeW });
  }
  if (typeof before.freeH === 'number' && typeof after.freeH === 'number') {
    if (before.scrollV) { if (after.freeH < before.freeH - REGRESSION_EPS) scrollRegressions.push({ id: before.id, name: before.name, axis: 'H', before: before.freeH, after: after.freeH, delta: r2(after.freeH - before.freeH) }); }
    else if (after.freeH < -0.5) newOverflow.push({ id: before.id, name: before.name, axis: 'H', before: before.freeH, after: after.freeH });
  }
}
c.noNewOverflow = newOverflow.length === 0;   // scroll 이 아닌 컨테이너만 대상 — 실패하면 rollback
c.noErrors = errors.length === 0;
if (scrollRegressions.length) notes.push('scroll 컨테이너 중 free space 가 기존보다 나빠진 것 ' + scrollRegressions.length + '개 — typography 영향 가능성, rollback 유발 안 함: ' + JSON.stringify(scrollRegressions));

const failedCriteria = Object.keys(c).filter(k => c[k] !== true);
if (failedCriteria.length) return await rollback('되읽기 검증 실패: ' + failedCriteria.join(', '), { checks: c, newOverflow, scrollRegressions, inheritFail: inheritFail.slice(0, 30) });
...
try {
  figma.root.setPluginData(BASELINE_KEY, JSON.stringify({
    ...
    layoutBefore, layoutAfter, scrollRegressions, scopeImpact
  }));
...
return out({
  ...
  layoutBefore, layoutAfter, newOverflow, scrollRegressions, scopeImpact,
  mutationCount, notes, errorCount: errors.length, errors
});
