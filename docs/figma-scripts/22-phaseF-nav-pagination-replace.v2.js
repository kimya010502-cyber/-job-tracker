 ...
 *
 * v2: footer 세로 영향을 예측에 넣었다. Pagination 28→32 로 HUG footer 높이가 바뀐다.
 *     높이 변화 자체는 실패가 아니고, 그로 인한 새 넘침·잘림·y 이동·세로 충돌만 막는다.
 *     Phase E 에서 이미 있던 겹침(existingDebt)과 Phase F 가 새로 만드는 것(newPhaseFRegression)을 나눈다.
 *     Prev/Next 의 TEXT 숨김은 사람이 승인했다 (BOOLEAN 속성이 없어 노드 visible 을 직접 override).
 * ========================================================================== */

const SCRIPT_VERSION = '22-v2-phaseF-nav-pagination-replace';
...
if (footerNode && pagParent && pagPrediction) {
  ...
  if (pagIdx >= 0 && xsAfter) {
    ...
    footerImpact.footerHeightNote = '세로 영향은 footerVerticalImpact 에서 예측한다 — ' + (F.layoutSizingVertical === 'HUG'
...
/* ---------- footer 세로 영향 ----------
 * Pagination 높이가 28 → 32 로 바뀌면 HUG footer 의 높이도 바뀐다.
 * 높이가 느는 것 자체는 의도된 결과라 실패가 아니다.
 * 판정하는 것은 그 때문에 "새로" 생기는 것이다: 부모 밖으로 넘침, 잘림, footer 의 예상 못 한 y 이동,
 * 다른 형제와의 새 세로 충돌. Phase E 에서 이미 있던 겹침은 existingDebt 로 따로 둔다. */
const GRID_ID_FOR_DEBT = '1002:140';
const footerVerticalImpact = { resolved: false, issues: [] };
if (footerNode && pagParent && pagPrediction) {
  const F = layoutOf(footerNode);
  const flow = kids(footerNode).filter(c => c.visible !== false && safeGet(c, 'layoutPositioning') !== 'ABSOLUTE');
  const pagH0 = pagParent.geometry.height;
  const pagH1 = Number(String(pagPrediction.after).split('×')[1]);
  const pagW1 = Number(String(pagPrediction.after).split('×')[0]);
  const hOf = (c, after) => (after && c.id === PAG_PARENT) ? pagH1 : r2(c.height);
  const fh0 = r2(footerNode.height);

  /* 1) footer 자신의 높이 */
  let modelH = null, basisH = null;
  if (F.layoutSizingVertical === 'HUG' && F.layoutMode === 'HORIZONTAL') {
    modelH = after => r2(F.paddingTop + F.paddingBottom + Math.max.apply(null, flow.map(c => hOf(c, after))));
    basisH = 'HORIZONTAL + 세로 HUG — 가장 높은 자식 + 위아래 padding';
  } else if (F.layoutSizingVertical === 'HUG' && F.layoutMode === 'VERTICAL') {
    modelH = after => r2(F.paddingTop + F.paddingBottom + flow.map(c => hOf(c, after)).reduce((a, b) => a + b, 0) +
      (F.primaryAxisAlignItems === 'SPACE_BETWEEN' ? 0 : F.itemSpacing * Math.max(0, flow.length - 1)));
    basisH = 'VERTICAL + 세로 HUG — 자식 높이 합 + 간격 + padding';
  } else {
    modelH = () => fh0;
    basisH = '세로가 ' + F.layoutSizingVertical + ' — 높이는 바뀌지 않는다';
  }
  const fh0Model = modelH(false), fh1 = modelH(true);
  const dh = r2(fh1 - fh0);
  footerVerticalImpact.heightBasis = basisH;
  footerVerticalImpact.footerHeightBefore = fh0;
  footerVerticalImpact.footerHeightModelNow = fh0Model;
  footerVerticalImpact.modelReproducesCurrentHeight = near(fh0Model, fh0, 1);
  footerVerticalImpact.footerHeightPredictedAfter = fh1;
  footerVerticalImpact.heightDelta = dh;
  footerVerticalImpact.paginationHeightBefore = pagH0;
  footerVerticalImpact.paginationHeightAfter = pagH1;

  /* 2) footer 안 자식의 세로 정렬 — 지금 y 를 재현하는지 먼저 본다 */
  const yIn = (h, H) => {
    const inner = H - F.paddingTop - F.paddingBottom;
    if (F.layoutMode !== 'HORIZONTAL') return null;
    if (F.counterAxisAlignItems === 'CENTER') return r2(F.paddingTop + (inner - h) / 2);
    if (F.counterAxisAlignItems === 'MAX') return r2(F.paddingTop + inner - h);
    return r2(F.paddingTop);
  };
  footerVerticalImpact.counterAxisAlignItems = F.counterAxisAlignItems;
  footerVerticalImpact.childrenAfter = flow.map(c => ({ id: c.id, name: c.name, isPagination: c.id === PAG_PARENT,
    yBefore: r2(c.y), yModelNow: yIn(hOf(c, false), fh0), yAfter: yIn(hOf(c, true), fh1),
    heightAfter: hOf(c, true) }));
  footerVerticalImpact.childYModelReproducesCurrent = F.layoutMode === 'HORIZONTAL' &&
    footerVerticalImpact.childrenAfter.every(x => near(x.yModelNow, x.yBefore, 1));
  const pagRow = footerVerticalImpact.childrenAfter.filter(x => x.isPagination)[0];
  footerVerticalImpact.paginationYBefore = pagRow ? pagRow.yBefore : null;
  footerVerticalImpact.paginationYAfter = pagRow ? pagRow.yAfter : null;

  /* 3) footer 의 y — 부모 구조나 constraints 로 정한다 */
  const fp = footerNode.parent;
  const positioning = safeGet(footerNode, 'layoutPositioning');
  const cons = safeGet(footerNode, 'constraints');
  const fy0 = r2(footerNode.y);
  let fy1 = null, basisY = null;
  const parentAuto = fp && (safeGet(fp, 'layoutMode') === 'VERTICAL' || safeGet(fp, 'layoutMode') === 'HORIZONTAL');
  if (fp && parentAuto && positioning !== 'ABSOLUTE') {
    const pm = safeGet(fp, 'layoutMode');
    if (pm === 'HORIZONTAL') {
      const a = safeGet(fp, 'counterAxisAlignItems');
      fy1 = a === 'CENTER' ? r2(fy0 - dh / 2) : (a === 'MAX' ? r2(fy0 - dh) : fy0);
      basisY = '부모 HORIZONTAL Auto Layout, counterAxis ' + a;
    } else {
      const a = safeGet(fp, 'primaryAxisAlignItems');
      const hugV = safeGet(fp, 'layoutSizingVertical') === 'HUG';
      fy1 = (hugV || a === 'MIN') ? fy0 : (a === 'CENTER' ? r2(fy0 - dh / 2) : (a === 'MAX' ? r2(fy0 - dh) : null));
      basisY = '부모 VERTICAL Auto Layout, primaryAxis ' + a + (hugV ? ', 부모 세로 HUG' : '');
      if (fy1 === null) footerVerticalImpact.issues.push('부모 정렬 ' + a + ' 에서는 footer y 를 예측하지 않는다');
    }
  } else {
    const v = cons ? cons.vertical : null;
    if (v === 'MIN' || v === 'TOP') fy1 = fy0;
    else if (v === 'CENTER') fy1 = r2(fy0 - dh / 2);
    else if (v === 'MAX' || v === 'BOTTOM') fy1 = r2(fy0 - dh);
    else footerVerticalImpact.issues.push('constraints.vertical = ' + v + ' 에서는 footer y 를 예측하지 않는다');
    basisY = (positioning === 'ABSOLUTE' ? 'ABSOLUTE' : '부모가 Auto Layout 아님') + ' — constraints.vertical = ' + v;
  }
  footerVerticalImpact.footerLayoutPositioning = positioning;
  footerVerticalImpact.footerConstraints = cons;
  footerVerticalImpact.yBasis = basisY;
  footerVerticalImpact.footerYBefore = fy0;
  footerVerticalImpact.footerYPredictedAfter = fy1;
  footerVerticalImpact.footerYShift = fy1 === null ? null : r2(fy1 - fy0);
  footerVerticalImpact.footerBottomBefore = r2(fy0 + fh0);
  footerVerticalImpact.footerBottomPredictedAfter = fy1 === null ? null : r2(fy1 + fh1);

  /* 4) 부모 기준 넘침 · 잘림 */
  if (fp) {
    const ph0 = r2(fp.height);
    const parentHug = parentAuto && positioning !== 'ABSOLUTE' && safeGet(fp, 'layoutSizingVertical') === 'HUG' &&
      safeGet(fp, 'layoutMode') === 'VERTICAL';
    const ph1 = parentHug ? r2(ph0 + dh) : ph0;
    footerVerticalImpact.footerParent = { id: fp.id, name: fp.name, size: geo(fp).size,
      layoutMode: safeGet(fp, 'layoutMode'), layoutSizingVertical: safeGet(fp, 'layoutSizingVertical'),
      clipsContent: safeGet(fp, 'clipsContent'), heightAfter: ph1, grows: parentHug };
    const ov0 = r2(Math.max(0, footerVerticalImpact.footerBottomBefore - ph0));
    const ov1 = footerVerticalImpact.footerBottomPredictedAfter === null ? null
      : r2(Math.max(0, footerVerticalImpact.footerBottomPredictedAfter - ph1));
    footerVerticalImpact.parentOverflowBefore = ov0;
    footerVerticalImpact.parentOverflowAfter = ov1;
    footerVerticalImpact.newParentOverflowPx = ov1 === null ? null : r2(ov1 - ov0);
    footerVerticalImpact.newClipping = safeGet(fp, 'clipsContent') === true && ov1 !== null && ov1 - ov0 > 0.5;

    /* 5) 부모 안 다른 형제와의 세로 충돌 — 전후 비교 */
    const fx0 = r2(footerNode.x), fx1 = r2(footerNode.x + footerNode.width);
    footerVerticalImpact.siblings = kids(fp).filter(c => c.id !== FOOTER_ID && c.visible !== false).map(c => {
      const hx = r2(c.x) < fx1 - 0.5 && fx0 < r2(c.x + c.width) - 0.5;
      let bottom = r2(c.y + c.height), contentBottom = null;
      if (c.id === GRID_ID_FOR_DEBT && kids(c).length) {
        contentBottom = r2(c.y + Math.max.apply(null, kids(c).filter(k => k.visible !== false).map(k => k.y + k.height)));
        bottom = Math.max(bottom, contentBottom);
      }
      /* 방향이 있는 겹침 — Phase E 의 정의와 같다.
       *   형제가 footer 위쪽에서 시작하면: 형제 바닥 − footer 위 (footer 위가 움직일 때만 바뀐다)
       *   형제가 footer 위 이후에서 시작하면: footer 바닥 − 형제 위 (footer 바닥이 내려갈 때 바뀐다)
       * 겹친 구간 길이로 재면 footer 가 아래로 자랄 때 위쪽 형제와의 겹침이 늘어난 것처럼 잘못 보인다. */
      const above = r2(c.y) < fy0 - 0.5;
      const intrude = (top, bot) => above ? r2(Math.max(0, bottom - top)) : r2(Math.max(0, bot - r2(c.y)));
      const before = hx ? intrude(fy0, footerVerticalImpact.footerBottomBefore) : 0;
      const after = (hx && fy1 !== null) ? intrude(fy1, footerVerticalImpact.footerBottomPredictedAfter) : null;
      return { id: c.id, name: c.name, horizontalOverlap: hx, bottomUsed: bottom, relation: above ? '형제가 footer 위쪽에서 시작' : '형제가 footer 위 이후에서 시작',
        contentBottom, overlapBefore: before, overlapAfter: after,
        newOverlapPx: after === null ? null : r2(after - before),
        isKnownPhaseEDebt: c.id === GRID_ID_FOR_DEBT };
    });
    footerVerticalImpact.newVerticalCollisions = footerVerticalImpact.siblings.filter(s => s.newOverlapPx === null || s.newOverlapPx > 0.5);
  } else footerVerticalImpact.issues.push('footer 부모를 읽지 못했다');

  footerVerticalImpact.existingDebt = (footerVerticalImpact.siblings || []).filter(s => s.isKnownPhaseEDebt)
    .map(s => ({ id: s.id, overlapBefore: s.overlapBefore, overlapAfter: s.overlapAfter,
      note: 'Phase E 에서 이미 있던 grid/footer 겹침. Phase F 실패로 보지 않는다. 늘어나는 양만 newPhaseFRegression 으로 본다' }));
  footerVerticalImpact.newPhaseFRegression = {
    parentOverflowPx: footerVerticalImpact.newParentOverflowPx,
    clipping: footerVerticalImpact.newClipping,
    footerYShift: footerVerticalImpact.footerYShift,
    newCollisions: (footerVerticalImpact.newVerticalCollisions || []).map(s => ({ id: s.id, name: s.name, newOverlapPx: s.newOverlapPx }))
  };
  footerVerticalImpact.resolved = footerVerticalImpact.issues.length === 0 &&
    footerVerticalImpact.modelReproducesCurrentHeight === true &&
    footerVerticalImpact.childYModelReproducesCurrent === true &&
    footerVerticalImpact.footerYPredictedAfter !== null && !!footerVerticalImpact.footerParent;
}
...
const preflight = {
  ...
  footerVerticalImpactResolved: footerVerticalImpact.resolved === true,
  footerHeightPredictionResolved: typeof footerVerticalImpact.footerHeightPredictedAfter === 'number',
  footerHeightModelReproducesCurrent: footerVerticalImpact.modelReproducesCurrentHeight === true &&
    footerVerticalImpact.childYModelReproducesCurrent === true,
  noNewFooterParentOverflow: typeof footerVerticalImpact.newParentOverflowPx === 'number' && footerVerticalImpact.newParentOverflowPx <= 0.5,
  noNewVerticalClipping: footerVerticalImpact.newClipping === false,
  noUnexpectedFooterYShift: typeof footerVerticalImpact.footerYShift === 'number' && Math.abs(footerVerticalImpact.footerYShift) < 0.5,
  noNewVerticalCollision: Array.isArray(footerVerticalImpact.newVerticalCollisions) && footerVerticalImpact.newVerticalCollisions.length === 0,
...
const decisionsToConfirm = [{
  approved: true,
  approvedNote: '사람이 승인했다 — BOOLEAN 속성이 없으므로 인스턴스 안 TEXT 노드 visible 을 직접 false 로 override',
...
if (DRY_RUN) {
  return out({
    ...
    footerVerticalImpact,
    existingDebt: footerVerticalImpact.existingDebt,
    newPhaseFRegression: footerVerticalImpact.newPhaseFRegression,
...
const baseline = {
  ...
  footerPrediction: {
    heightBefore: footerVerticalImpact.footerHeightBefore, heightAfter: footerVerticalImpact.footerHeightPredictedAfter,
    yBefore: footerVerticalImpact.footerYBefore, yAfter: footerVerticalImpact.footerYPredictedAfter,
    bottomBefore: footerVerticalImpact.footerBottomBefore, bottomAfter: footerVerticalImpact.footerBottomPredictedAfter,
    paginationYAfter: footerVerticalImpact.paginationYAfter,
    parentId: footerVerticalImpact.footerParent ? footerVerticalImpact.footerParent.id : null,
    parentHeightAfter: footerVerticalImpact.footerParent ? footerVerticalImpact.footerParent.heightAfter : null,
    parentOverflowBefore: footerVerticalImpact.parentOverflowBefore, parentOverflowAfter: footerVerticalImpact.parentOverflowAfter,
    siblingsOverlapAfter: (footerVerticalImpact.siblings || []).map(x => ({ id: x.id, overlapAfter: x.overlapAfter, isKnownPhaseEDebt: x.isKnownPhaseEDebt }))
  },
  replacements: {}
};
...
const successCriteria = {
  ...
  footerHeightAsPredicted: !!footerNode && near(footerNode.height, footerVerticalImpact.footerHeightPredictedAfter, 0.5),
  footerYAsPredicted: !!footerNode && near(footerNode.y, footerVerticalImpact.footerYPredictedAfter, 0.5),
...
return out({
  ...
  footerAfter: footerNode ? { size: geo(footerNode).size, y: r2(footerNode.y), predictedHeight: footerVerticalImpact.footerHeightPredictedAfter, predictedY: footerVerticalImpact.footerYPredictedAfter } : null,
