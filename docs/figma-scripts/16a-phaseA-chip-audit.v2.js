/* ============================================================================
 * 줍줍 — 스크립트 16a v2
 * Phase A 사전 감사 (읽기 전용) — scanner 보정 + 부모 높이 영향 재계산
 *
...
const SCRIPT_VERSION = '16a-v2-scanner-and-parent-growth';
...
function predicates(n) {
  ...
  return {
    ...
    wideEnough: ratio !== null && ratio >= 1.3,
    _ratio: ratio,
    _height: r2(n.height),
...
function shapeOk(pr) {
  return pr.isFrameOrGroup && pr.heightInRange && pr.childCountInRange &&
         pr.hasTextWithin2 && pr.hasVisiblePaint && pr.hasCornerRadius && pr.wideEnough;
}
...
const extraInScan = scanIds.filter(id => candIds.indexOf(id) < 0);
...
const gate = {
  ...
  scanCountIs3: scanCount === 3,
  scanMatchesCandidateCount: scanCount === CANDIDATES.length,
  scanExactlyTheThreeCandidates: missingFromScan.length === 0 && extraInScan.length === 0
};
...
return out({
  ...
  notes,
  nextStep: 'gatePassed = true 이면 tone 매핑을 확정하고 DRY_RUN 교체 스크립트(16)와 검증기(16b)를 씁니다.'
});
